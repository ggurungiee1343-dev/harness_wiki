---
title: "rohitg00/agentmemory: #1 Persistent memory for AI coding agents based on real-world benchmarks"
source: "https://github.com/rohitg00/agentmemory"
author:
  - "[[rohitg00]]"
published:
created: 2026-06-07
description: "AI 코딩 에이전트가 이전 작업 내용을 기억하도록 하는 지속 메모리 시스템을 제공한다. Claude Code, GitHub Copilot CLI, Cursor 등 다양한 코딩 도구와 연동하여 재설명 없이 작업을 이어갈 수 있다. 실제 벤치마크 기반 구현으로 생산성을 크게 향상시킨다."
tags: [ingested, 10_AI_Automation, ai, coding-agent, persistent-memory, github, claude-code, automation]
  - "clippings"
brief: "brief"
---
...
![1,423+ tests passing](https://github.com/rohitg00/agentmemory/raw/main/assets/tags/stat-tests.svg)
...
| **OpenClaw** | OpenClaw MCP config | Same `mcpServers` block, or use the deeper [memory plugin](https://github.com/rohitg00/agentmemory/blob/main/integrations/openclaw). |
...
| **OpenCode (full plugin)** | `plugin/opencode/` | 22 auto-capture hooks covering session lifecycle, messages, tools, errors. Two slash commands (`/recall`, `/remember`). Copy `plugin/opencode/` into your OpenCode workspace and add the plugin entry to `opencode.json`. See [`plugin/opencode/README.md`](https://github.com/rohitg00/agentmemory/blob/main/plugin/opencode/README.md) for the full hook table + gap analysis. |
| **pi** | `~/.pi/agent/extensions/agentmemory` | Copy [`integrations/pi`](https://github.com/rohitg00/agentmemory/blob/main/integrations/pi) and restart pi. |
| **Hermes Agent** | `~/.hermes/config.yaml` | Use the deeper [memory provider plugin](https://github.com/rohitg00/agentmemory/blob/main/integrations/hermes) with `memory.provider: agentmemory`. |
...
### Multi-agent memory (AGENT\_ID + AGENTMEMORY\_AGENT\_SCOPE)
