...
| 날짜 | 분야 | 교훈 |
|---|---|---|
| 2026-07-08 | 인프라 fix는 형제 스크립트에도 이식 | `run_scan.py`에 넣은 fd 한도 fix(`resource.setrlimit`, BUG-MJS-023/026)가 완전히 별개 스크립트인 `batch_fill_returns.py`엔 안 들어가 있어서 같은 fd 고갈 크래시가 반복되고, 그 여파로 매수→매도 시그널 추적(`signal_tracker.update_positions()`)이 몇 달째 전혀 작동 안 함(전량 status=active). 한 곳에서 고친 인프라 버그는 같은 자원을 쓰는 다른 진입점에도 즉시 grep해서 이식할 것. 06번 BUG-MJS-029 참조. |
| 2026-07-08 | "0건/미작동"을 추정으로 설명 금지 | "SELL_CONFIRMED가 아직 안 걸린 건 데이터가 어려서(11일밖에 안 지나서)"라고 근거 없이 답했다가 MJ님이 "과거에 매도신호 떴었는데 못 본 것 같다"고 반박 — 실제로는 트래커가 10일간 멈춰있어 과거 신호를 놓치고 오늘 늦게 잡은 것(circle_date 소급 왜곡)이 진짜 원인이었음. 시스템이 0건/미작동일 때 반드시 로그·코드로 원인 확인 후 답할 것. 06번 BUG-MJS-029 참조. |
| 2026-07-08 | 새 도구 제안 전 docs 먼저 확인 | "매수→매도 사이클 추적 시스템 없다"며 신규 백테스트 도구를 제안했으나, MJ님 지적으로 `docs/quant_logic_analysis.html`을 보니 `signal_tracker.py` 이벤트 기반 추적이 이미 설계돼 있었음(단지 크론 크래시로 안 돌고 있었을 뿐). 로직/프로세스 질문은 새로 만들자고 하기 전에 docs 폴더 html부터 확인할 것. 06번 BUG-MJS-029 참조. |
| 2026-07-08 | 검색기 "0건 지속" 진단법 | 며칠째 0건이어도 에러 로그는 깨끗할 수 있음(침묵 실패) — 조건식을 개별 컬럼으로 분해해 실제 KIS 데이터로 조건별 통과율(%)을 실측하는 진단 스크립트가 육안 코드 리뷰보다 훨씬 빨리 범인을 좁힘. `diagnose_zero_hit_screeners.py` 패턴 재사용. 06번 BUG-MJS-028 참조. |
| 2026-07-08 | 문서가 최신이라는 가정은 위험 | `korean_original_formulas.html`과 `kr_to_us_conversion.html`이 같은 조건(pochak `c`/`E`)을 서로 다르게 기록하고 있었음 — 코드-문서 불일치 시 문서도 틀렸을 수 있으므로 실물 HTS 캡처로 재확인 필요. 여러 문서가 있으면 전부 대조할 것. 06번 BUG-MJS-028 참조. |
| 2026-07-08 | 필드 오비교와 윈도우 오류는 다른 버그 클래스 | `close` vs `high` 같은 필드 비교 오류와 EMA200 vs EMA115 같은 윈도우(파라미터) 오류는 증상(조건 항상 False)은 같아도 원인 레이어가 다름 — 진단 시 항목별로 분리 확인하고, 하나 고쳤다고 다른 것도 고쳐졌다고 가정 금지. 06번 BUG-MJS-028 참조. |
...
| 2026-07-06 | 리팩터링·파일삭제는 호출부/import 전수 grep + 재시작 검증 | cove_engine `process_query`→`run_cove_pipeline` 교체 후 호출부 5곳 미갱신으로 `보안 리뷰`·`/ask`·`/devil` 전멸. verification_engine.py는 삭제됐는데 `_base.py`가 계속 import(실행 프로세스 메모리 캐시로만 생존 = 재시작 지뢰). **lazy-load 핸들러는 봇 기동≠명령 동작 — 클릭해야 터짐.** 06번 BUG-CORE-022 참조. |
...
| 2026-07-04 | ~~외부 의존 자원 장애는 "빌려주는 쪽" 상태부터 확인~~ | **[CLAUDE.md §"내 통제 범위는 깨끗함 = 멈출 지점이 아니라 확장할 신호"로 승격 완료 — 표에서 삭제]** KIS API 사례(지난 SNI 차단과 성급히 동일 일반화 → 실제는 KIS 자체 점검 공지)가 계기. MJ님이 한 단계 더 짚어준 확장판: "내 안 깨끗하면 거기서 멈추지 말고 논리적으로 밖까지 계속 넓혀가라, 모르겠다로 끝내지 마라." |
